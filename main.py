...
storage_client = storage.Client.from_service_account_json(
    "sacred-epigram-411001-8bba796e9384.json"
)
...
async def test_command(ctx):
    ...
    if result and is_enabled:
        ...
        if not await send_patch_message(
            patch_title, patch_url, patch_img, channel_list, True
        ):
        ...
        message = (
            f"{called_channel_url} チャンネルにテストメッセージを2件送信しました！"
        )
...
async def send_patch_message(
    patch_title, patch_url, patch_img=None, enabled_channel_list=None, is_test=False
):
...
async def send_dev_message(new_dev_info_list, enabled_channel_list=None, is_test=False):
    ...
    for new_dev_info in new_dev_info_list:
        for channel_id in enabled_channel_list:
            ...
            try:
                await channel.send(
                    f"### - [{new_dev_info['title']}]({new_dev_info['url']})"
                )
                if is_test:
                    return True
            except:
